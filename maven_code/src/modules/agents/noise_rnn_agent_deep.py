...
class RNNAgentDeep(nn.Module):
    def __init__(self, input_shape, args):
        super(RNNAgentDeep, self).__init__()
        self.args = args
        ...
        if self.args.nn_activation == "relu":
            self.activation = F.relu
            self.act_fun = th.nn.ReLU
        elif self.args.nn_activation == "tanh":
            self.activation = F.tanh
            self.act_fun = th.nn.Tanh
        elif self.args.nn_activation == "elu":
            self.activation = F.elu
            self.act_fun = th.nn.ELU
        else:
            raise NotImplementedError(print("Only relu, tanh and elu supported atm"))
        ...
        self.hyper_noise_fc1 = th.nn.Sequential(
            nn.Linear(args.noise_dim + args.n_agents, args.rnn_hidden_dim * 2),
            self.act_fun(),
            nn.Linear(args.rnn_hidden_dim * 2, args.rnn_hidden_dim * 2),
            self.act_fun(),
            nn.Linear(args.rnn_hidden_dim * 2, args.rnn_hidden_dim * args.n_actions)
        )
